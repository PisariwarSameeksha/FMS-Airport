...
public class AirportServiceImpl implements AirportService {
	...
	@Override
	public String addAirport(AirportDTO newAirportDTO) throws AirportAlreadyExistsException {

		Airport airportName = this.airportRepository.findByAirportName(newAirportDTO.getAirportName());
		if (airportName == null) {
			Airport airport = modelMapper.map(newAirportDTO, Airport.class);
			airportRepository.save(airport);
			return "new Airport successfully added";
		} else {
			throw new AirportAlreadyExistsException("Airport name already exist!");
		}
	}
	...
	public List<AirportDTO> getAllAirports() {
		...
		List<AirportDTO> airportDTO = airport.stream().map(list -> modelMapper.map(list, AirportDTO.class))
